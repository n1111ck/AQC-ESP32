...
#endif
  m_wire->endTransmission();
  m_wire->requestFrom(address, (byte)1);
#if ARDUINO >= 100
  value = m_wire->read();
#else
  value = m_wire->receive();
#endif
  m_wire->endTransmission();
...
void Adafruit_HMC5883_Unified::read() {
  ...
  m_wire->beginTransmission((byte)HMC5883_ADDRESS_MAG);
#if ARDUINO >= 100
  m_wire->write(HMC5883_REGISTER_MAG_OUT_X_H_M);
#else
  m_wire->send(HMC5883_REGISTER_MAG_OUT_X_H_M);
#endif
  m_wire->endTransmission(false);
  m_wire->requestFrom((byte)HMC5883_ADDRESS_MAG, (byte)6, true);

// Note high before low (different than accel)
#if ARDUINO >= 100
  uint8_t xhi = m_wire->read();
  uint8_t xlo = m_wire->read();
  uint8_t zhi = m_wire->read();
  uint8_t zlo = m_wire->read();
  uint8_t yhi = m_wire->read();
  uint8_t ylo = m_wire->read();
#else
  uint8_t xhi = m_wire->receive();
  uint8_t xlo = m_wire->receive();
  uint8_t zhi = m_wire->receive();
  uint8_t zlo = m_wire->receive();
  uint8_t yhi = m_wire->receive();
  uint8_t ylo = m_wire->receive();
#endif
